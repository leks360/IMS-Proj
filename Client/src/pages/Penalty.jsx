...
import AddPenalty from '../components/addPenalty.jsx';
...
export default function Levies(props) {
    ...
    useEffect(()=>{
       const fetch=async()=>{
        ...
        //getAllPaginated
        const da=await axiosInstance.get(`/penalty/getAllPaginated/${ser}?page=${page}&limit=${limit}`);
    ...
    return (
        ...
        <Header/>
            <Tit>
                <H3>Penalties Imposed</H3>
            </Tit>
            {openCreate && ser!='none' && 
            <CreateDiv> 
               <Card>
               <Btn onClick={e=>setOpen(false)}>X</Btn>
                <AddPenalty name={ser} setOpen={setOpen} setReload={setReload}/>
                </Card>
            ...
            <Inputs>
                ...
                <ButtonTemplate onClick={(e)=>{ser!='none' && setOpen(true)}}>Add Penalty</ButtonTemplate>
            </Inputs>
            
            
        {data!=null &&<BasicTable dataa={data} setLimit={setLimit} setPage={setPage} page={page} which="penalty"/>}
        </Wrapper>
